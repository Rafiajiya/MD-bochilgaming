...
export const listJadwalSholat = JSON.parse(readFileSync(join(__dirname, "../../data/jadwal-sholat.json"), "utf-8"));
export default async function jadwalsholat(kota) {
    let jadwal;
    if (!(jadwal = listJadwalSholat.find(({ kota: Kota }) => new RegExp(Kota, "ig").test(kota))))
        throw new ScraperError("List kota " + listJadwalSholat.map(({ kota }) => kota));
