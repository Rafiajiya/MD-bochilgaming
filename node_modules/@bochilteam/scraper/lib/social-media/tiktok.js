...
export async function tiktokdl(url) {
    ...
    let res = await got(`https://api.snaptik.site/video-key?video_url=${url}`).json();
    if (res["status"] != "success")
        throw new ScraperError("Failed to get TikTok video key");
    let data = await got(`https://api.snaptik.site/video-details-by-key?key=${res["data"]["key"]}`).json();
