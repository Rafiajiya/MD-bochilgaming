...
export async function lyricsv2(query) {
    ...
    const { artist_names, title, url } = data.response.sections.find(({ type, hits }) => ['song', 'lyric'].includes(type) &&
        hits.find(({ type }) => ['song', 'lyric'].includes(type))).hits.find(({ type }) => ['song', 'lyric'].includes(type)).result;
    if (!url)
